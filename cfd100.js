...
[task_local]
#财富岛100
0 * * * * jd_car_exchange.js, tag=财富岛100, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_redPacket.png, enabled=true

================Loon==============
[Script]
cron "0 * * * *" script-path=jd_car_exchange.js, tag=财富岛100

===============Surge=================
财富岛100 = type=cron,cronexp="0 * * * *",wake-system=1,timeout=3600,script-path=jd_car_exchange.js

============小火箭=========
财富岛100 = type=cron,script-path=jd_car_exchange.js, cronexpr="0 * * * *", timeout=3600, enable=true
 */
const $ = new Env('财富岛5兑换');
...
function exchange() {
  var options = {
    ...
    'headers': {
      ...
      'referer': 'https://st.jingxi.com/fortune_island/index2.html?ptag=138631.26.55&trace=1.CIifhITXufOoVRCjwv6KBiC8NjgKQAJI1wJQAQ',
      'accept-encoding': 'gzip, deflate',
      'accept-language': 'zh-CN,zh-HK;q=0.9,zh;q=0.8,en-CN;q=0.7,en-US;q=0.6,en;q=0.5',
      'cookie': cookie
    }
  };
  return new Promise(resolve => {
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if(JSON.stringify(data).indexOf('不足')>=0){
            console.log('财富不足10元，开始尝试兑换5元');
            var options2 = {
              'method': 'GET',
            ...
            $.get(options2,(err, resp, data)=>{
              if (err) {
                console.log(`${JSON.stringify(err)}`)
                console.log(`${$.name} user/exchange/bean API请求失败，请检查网路重试\n`)
              }else{
                if(JSON.stringify(data).indexOf('不足')>=0){
                  console.log('财富不足5元终止');
                }else{
